...
const CardCurso = ({ curso: { id, titulo, descripcion, imagen } }) => {
  ...
  const [carritoStateContext, agregarCarrito] = useContext(CarritoContext);
  ...
  return (
    <div className="container-curso m-6 figure-curso mb-6">
      <figure className="bg-slate-100 rounded-xl p-8 dark:bg-slate-800 figure-curso flex">
        <div className="flex-1 flex flex-col justify-between items-center">
          ...
          <div className=" flex flex-row justify-center">
           <Button title="Carrito" onPress={() => verificarSesion()}/>
          </div>
        </div>
      </figure>
    </div>
